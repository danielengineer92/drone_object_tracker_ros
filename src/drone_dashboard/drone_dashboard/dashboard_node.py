...
It can also publish an operator autonomy request through:
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .card {
      ...
      box-shadow: 0 20px 60px rgba(0,0,0,.38), inset 0 1px 0 rgba(255,255,255,.045);
...
<body>
      ...
      <div class="rightStack">
        ...
        <section class="card">
          ...
          <div class="gates">
            ...
            <div class="gate"><span>MAVSDK</span><span id="gateMavsdk" class="state warn">UNKNOWN</span></div>
          </div>
        </section>
  ...
  <script>
    ...
    async function setAutonomy(enabled) {
      await fetch('/api/autonomy_request', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ enabled }) });
      await refresh();
    }
    ...
    async function refresh() {
      try {
        ...
        const t = s.telemetry || {};
        const c = s.control || {};
        const mission = s.mission_state || s.autonomy_state || 'UNKNOWN';
        ...
        setText('gateMavsdk', s.mavsdk_status || 'UNKNOWN', statusClass(String(s.mavsdk_status).includes('SENT') || String(s.mavsdk_status).includes('OK'), true));
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802 - http.server API name
        if self.path != "/api/autonomy_request":
            self._send(404, b"not found", "text/plain")
            return
        ...
        request_fn: Callable[[bool], None] = self.server.request_autonomy_fn  # type: ignore[attr-defined]
        request_fn(enabled)
...
class DashboardNode(Node):
    def __init__(self) -> None:
        ...
        self.get_logger().warning(
            ...
            f"mission_state={self.mission_state_topic}, request_topic={self.autonomy_request_topic}"
        )
    ...
    def _target_cb(self, msg: TargetError) -> None:
        ...
        self.diagnostics.mark_received(self.target_error_topic, summary=f"state={msg.tracking_state}, visible={msg.target_visible}")
